...
use crate::{compute_fej::compute_fej, vk::VerifierProcessedInputs};
...
pub fn fflonk_verifier(vpi: VerifierProcessedInputs, proof: Proof, pub_input: &Fr) {
    ...
    let inv_tuple = Inversion::build(&proof, &challenges, &roots);
    ...
    let (R0, R1, R2) = compute_r(&proof, &challenges, &roots, &inv_tuple, &pi);
